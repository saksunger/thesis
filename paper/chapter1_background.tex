...
tie each concept either to the dataset of Chapter~\ref{ch:data} or to the
...
handover events to be analysed (Chapter~\ref{ch:data}).
...
19-cell two-tier hexagonal layout is treated as 19 distinct gNBs, so that by
...
is described in Chapter~\ref{ch:data}.
...
\begin{itemize}
  ...
  reference signal, in dBm; the layer-3 reporting range is
  $-156$ to $-31$\,dBm with 1\,dB resolution~\cite{ts38215,ts38133}. RSRP is the primary
...
Chapter~\ref{ch:data} generates handover attempts, successes, failures, the
...
Surveys organise anomaly detectors by the principle used to model
normality~\cite{chandola2009anomaly}; for the methods compared in this thesis it
is convenient to distinguish statistical/probabilistic methods, distance- and
density-based methods, boundary- or domain-based methods, isolation/ensemble
methods, and reconstruction-based methods (including, more recently, deep
autoencoders). The five detectors
...
anomalies are ``few and different''~\cite{liu2008isolation}, a tree isolates
them after only a few splits,
...
score; this is closely related to early PCA-based detectors that score anomalies
with statistics over the major and minor principal
components~\cite{shyu2003pca}. Normal points lie close to the subspace and reconstruct
...
day/night or seasonal patterns). Different detector families are sensitive to
different shapes, which is one reason the benchmark covers several.
...
between the two empirical cumulative distribution functions; the underlying KS
statistic is the classical goodness-of-fit measure popularised by
Massey~\cite{massey1951ks}, used here in its two-sample form.
In this thesis the KS statistic is used to calibrate the simulator against real
measurements (Chapter~\ref{ch:data}), not as a runtime drift detector.
...
\begin{itemize}
  ...
  differ~\cite{raab2020kswin}; it is the streaming counterpart of the offline KS
  test above.
\end{itemize}

The second family is \emph{error-rate (performance-based)} and watches a binary
signal (here, a binarised version of the KPI stream: whether each sample
deviates from a running mean by more than a z-score threshold), reacting when
the rate of deviations worsens:
\begin{itemize}
  ...
  rate climbs past calibrated thresholds~\cite{gama2004ddm,gama2014survey}.
  ...
  drift~\cite{baenagarcia2006eddm}.
\end{itemize}
...
\begin{itemize}
  ...
  two samples in a kernel feature space~\cite{gretton2012mmd}.
  \item \textbf{Energy distance} measures it from the pairwise Euclidean
  distances within and between the two samples~\cite{szekely2013energy}.
\end{itemize}
...
energy distance). The same de-duplication rule as in
Section~\ref{subsec:anomaly-methods} applies: HDDM is another error-rate
detector alongside DDM/EDDM, and
...
3GPP-compliant simulator described in Chapter~\ref{ch:data}, whose radio layer
...
optimization in 5G and beyond networks~\cite{hoMgmtSurvey2025}. Two strands of
prior work matter here: (i) AI-based handover \emph{prediction and control},
which tries to act on handovers before they go wrong, and (ii) handover
\emph{anomaly detection} together with \emph{cellular KPI concept drift}, which
watches handover behaviour for trouble. Each is valuable on its own, but their integration into a
single handover-specific monitoring framework remains limited.

\section{AI-based handover prediction and control}
...
\section{Handover anomaly detection and cellular KPI drift}
...
support. One work places anomaly detection at two different points of the 5G
O-RAN handover decision: a serving-side detector watches each user's KPIs for
early signs of throughput trouble (a trigger for proactive handover), while a
neighbour-side detector screens the candidate cells and discards those whose
radio conditions look abnormal, shrinking the candidate set by roughly
40\%~\cite{oranAnomaly2025}. A complementary benchmark in the O-RAN
traffic-steering module pits three unsupervised detectors (Local Outlier
Factor, Isolation Forest, Robust Covariance) against four supervised
classifiers (Logistic Regression, Naive Bayes, SVM, Random Forest) and reports
Random Forest as the strongest for handover prediction~\cite{oranBenchmark2023}. These works show that KPI-focused
...
On the drift side, the most relevant work is LEAF~\cite{leaf}. Analysing more
than four years of KPI data from a major US operator, the study establishes
three facts that matter for this thesis: cellular KPI models age, whatever
model family or training-set size is chosen; the obvious remedy of refitting on
fresh data is unreliable and at times leaves the model worse off; and a
targeted response (find the drift, identify the features that carry it, then
retrain with forgetting and over-sampling) recovers accuracy where blind
refits do not. This is strong
...
\section{Literature matrix and research gap}
...
Isolation Forest, Random Forest, AutoEncoder, AE-1SVM; serving/neighbour-cell KPIs &
...
A-MMUF framework~\cite{ammuf2022} & 2023 &
...
Random-Forest forecasting $+$ PPO actor--critic cell ranking (rApp); HO execution in the xApp &
...
robustness optimization} (MRO), the SON function introduced for LTE and
specified for 5G management and orchestration in
TS~28.313~\cite{ts28313}. In MRO the function ``detects handover issues (e.g.\
too late HO, too early HO and HO to a wrong cell) in intra-RAT or inter-RAT
mobility by analysing reports from UEs and network side information, and acts to
mitigate the HO issues by adjusting HO related parameters''~\cite{ts28313},
...
Section~\ref{subsec:rw-prediction} and is left to future work. The
contribution here is therefore the \emph{monitoring} methodology, not the
...
an unfiltered refit can do more harm than no refit at all: drift-aware
retraining pays off only when the retraining pool is first cleaned by a
self-supervised filter against self-poisoning. On top of this
