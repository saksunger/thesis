...
Mobility management, and especially the handover (HO) process, is one of the key control functions in cellular communication systems. In 5G and future radio access networks (RANs), the growing number of users, dense small-cell deployments, higher carrier frequencies, and higher user mobility make the radio environment increasingly dynamic~\cite{hoMgmtSurvey2025}. As networks grow denser and more heterogeneous, each user performs more handovers, and an incorrectly configured or poorly monitored handover can lead to HO failures, ping-pong effects, or radio link failures (RLF) that directly degrade the quality of service experienced by the user equipment (UE)~\cite{tr36839}. Reliable handover monitoring is therefore becoming a central concern in modern mobile networks.
...
This thesis focuses specifically on the intra-RAT inter-gNB HO scenario, which is one of the most common and operationally important mobility cases in dense 5G deployments~\cite{hoMgmtSurvey2025}. Compared to inter-RAT transitions, intra-RAT handovers also offer a more homogeneous technical environment for the analysis pursued here~\cite{ts38300,ts37340}. The scenario is thus both practically important and tractable as a research target.
...
The remainder of this thesis is organized as follows. Chapter~\ref{ch:background} presents the background and related work. Chapter~\ref{ch:method} describes the datasets, the simulation environment, and the methodology. Chapter~\ref{ch:results} presents and discusses the experimental results. Finally, the last chapter summarizes the conclusions and outlines possible future research directions.