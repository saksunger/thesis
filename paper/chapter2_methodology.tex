...
\chapter{Data and Simulation Environment}
\label{ch:data}

This chapter sets out the research design, then describes how the experimental
data are produced and validated: the data sources, the custom 5G NR handover
simulator that generates the primary data, and the calibration of its radio
layer against real measurements. The methods that operate on this data, the
monitoring and modelling layers, are the subject of
Chapter~\ref{ch:method}. The central
...
Building on the data and simulation environment of Chapter~\ref{ch:data}, this
chapter describes how the monitoring and modelling layers are constructed and
evaluated, and which methods, metrics, and tools are used to address the four
research questions (RQ1--RQ4) set out in the Introduction. It first explains how
drift and anomalies are injected into the simulated timelines and how the static
configuration sweep is built, then details the four analysis layers (anomaly
detection, drift detection, drift-aware adaptive monitoring, and the
configuration--performance surrogate), the end-to-end loop that combines them,
the external-validation checks, and the implementation and reproducibility
setup.
