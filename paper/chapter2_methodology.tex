...
to address the research questions introduced in Section~\ref{sec:gap}. It also
...
the simulator rather than as the primary training source. The reason for this
...
anomaly detector when persistent drift is detected, and (iv) a
...
seven-cell layout is modelled as a distinct gNB, such that all handovers
...
monitored KPI family is restricted to \emph{mobility KPIs} as defined in
TS~28.554~\S6.3.1--6.3.2~\cite{ts28554}: handover success rate (HOSR), handover
failure rate, radio-link-failure (RLF) rate, and ping-pong rate, together with
...
\node[box] (s1) at (0,-1.8) {\textbf{Stage 1 -- Geometry}\\ seven-cell hex grid $+$ per-UE trajectory};
...
injection) drive three stages: \emph{geometry} (seven-cell hexagonal grid, each
...
The scenario is a seven-cell hexagonal grid (a centre cell plus the first ring of
six), with an inter-site distance of 500\,m and an omnidirectional antenna at
each cell centre. The default radio settings follow the 3GPP Urban Macro
reference deployment~\cite{tr38913}: carrier 3.5\,GHz (band n78), 100\,MHz
bandwidth carried on 273 resource blocks at 30\,kHz subcarrier
spacing~\cite{ts38104}, base-station height 25\,m, UE height 1.5\,m,
transmit power 49\,dBm, antenna gain 8\,dBi and a UE noise figure of 7\,dB. Each
of the seven cells is treated as a separate gNB, so every handover the simulator
...
the 3GPP TR~38.901 channel model~\cite{tr38901}. Three ingredients are combined.
...
(\texttt{scipy.stats.ks\_2samp})~\cite{massey1951ks}. The KS statistic, together
...
ROC-AUC reported alongside; precision--recall is preferred because anomalies are
rare. The task is in fact strongly imbalanced: at the 5\,s window the positive
...
Two model heads are used. A histogram gradient-boosting regressor
...
widen the nominal 5\%/95\% quantile band, and the result is a 90\% prediction
