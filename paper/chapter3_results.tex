...
$\approx 0.024$, so LOF's $0.152$ is roughly six times the chance level rather
than a non-detection. Slow degradation is the only type that stays essentially at
...
interest rather than a single headline number.

\paragraph{Robustness checks.}
The window-size sweep ($2$--$30$\,s) shifts the per-type numbers but not the
ranking: short windows ($2$\,s) help the abrupt glitch and burst, while a longer
window ($30$\,s) helps the slowly varying anomalies (for example RLF-burst PR-AUC
rises to $0.845$ for PCA-AE at $30$\,s). The drop-one-family ablation shows that
...
detectors (instance-level miss rate $0$), so the question is not \emph{whether}
drift is found but with what \emph{latency} and at what \emph{false-alarm} cost.
...
false alarm is higher. This trade-off, rather than a single best detector, is the
main observation for RQ2.

\section{Drift-aware adaptive monitoring (RQ3)}
\label{sec:res-adaptive}

The PCA-reconstruction autoencoder (PCA-AE) is used as the base monitor for the
...
monitor, and Figure~\ref{fig:adaptive} plots PR-AUC over time. The main takeaway
is that \emph{how} the monitor is retrained appears to matter more than
\emph{whether} it is retrained.
...
uplift is a model prediction rather than a measured field gain, which is stated
as a limitation.
...
Three checks probe how far these results travel
(Table~\ref{tab:external}). First, a cross-seed replication over five master
