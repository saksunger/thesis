...
data/processed/external_validation/                     # Phase 11 Iter A cross-seed validation:
...
| 11. External validation                         | **Iter A done** (cross-seed replication on `timeline_medium`, master_seed ∈ {42, 43, 44, 45, 46}; orchestrator + bootstrap-CI aggregator; 5 / 6 acceptance checks PASS. Headline findings: (a) **Phase 9 cumulative HOSR uplift reproduces to four decimal places across seeds** (+0.6214, 95 % CI [+0.6213, +0.6215]); (b) **filtered > naive drift-window PR-AUC replicates** (+0.072, 95 % CI [+0.033, +0.113]); (c) **drift detection volume CV = 0.04**; (d) anomaly winner is **Simpson's-paradox-split**: per-anomaly-type mean PR-AUC winner jitters across {LOF, OneClassSVM, MLP-AE} (top 4 detectors cluster within ≈ 0.03 PR-AUC), but **overall pooled PR-AUC winner is IsoForest on all 5 / 5 seeds**. Benchmark JSON now emits both `winning_detector_by_per_type_mean_pr_auc` and `winning_detector_by_overall_pr_auc`. Iter B (cross-scenario `timeline_dense_urban`) and Iter C (NordicDat face validity) pending.) 79 external-validation tests green. |
