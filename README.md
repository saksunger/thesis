...
| 11. External validation                         | **Iter A + B done** (Iter A: cross-seed replication on `timeline_medium`, master_seed ∈ {42..46}; 5 / 6 acceptance PASS. Iter B: cross-scenario on `timeline_dense_urban` (37 cells × 24 UEs); **5 / 5 acceptance PASS**. Headline findings: (a) **Phase 9 HOSR uplift reproduces to 4 decimals across seeds** (+0.6214 ± 0.0001) and **scales to +1.2413 in dense_urban** (≈ 2 ×); (b) **filtered > naive replicates in both scenarios** (medium +0.072 [95 % CI +0.033, +0.113], dense_urban +0.024); (c) drift detection volume CV = 0.04 across seeds, identical winner (Energy-batch) across scenarios; (d) **Simpson's paradox is scenario-conditional** — per-type-mean anomaly winner splits across {LOF, OneClassSVM, MLP-AE} on medium (top 4 cluster within ≈ 0.03 PR-AUC) but resolves to clean IsoForest dominance on dense_urban, while the overall pooled winner is IsoForest in BOTH scenarios. Benchmark JSON now emits both `winning_detector_by_per_type_mean_pr_auc` and `winning_detector_by_overall_pr_auc`. Iter C (NordicDat face validity) pending.) 79 external-validation tests green. |
